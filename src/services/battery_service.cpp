...
// ============================================================
// GPIO3 = ADC1_CH2 — припаян к делителю, НЕ меняем пин
// ============================================================
...
void battery_init() {
    // Полный сброс GPIO3 + отключение всех подтяжек
    ...
    // ADC напрямую, без Arduino обёртки
...
float battery_get_voltage() {
    ...
    // Перед каждым чтением — отключаем подтяжку заново
    // (некоторые драйверы могут её включить обратно)
    gpio_set_pull_mode((gpio_num_t)BAT_GPIO, GPIO_FLOATING);
